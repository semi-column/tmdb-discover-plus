...
class MetricsTracker {
  ...
  _cacheStatsProvider: (() => Record<string, unknown>) | null;
  ...
  middleware(): (req: Request, res: Response, next: NextFunction) => void {
    return (req: Request, res: Response, next: NextFunction) => {
      ...
      if (userId) {
        if (this.activeUsers.size < 50000 || this.activeUsers.has(userId)) {
   ...
   */
  trackProviderCall(provider: string, durationMs: number, isError: boolean = false): void {
  ...
  getSummary(): Record<string, unknown> {
    ...
    const endpointSummary: Record<string, unknown> = {};
  ...
  /** @private */
  _normalizeRoute(req: Request): string {
  ...
  /** @private */
  _recordEndpoint(route: string, durationMs: number, isError: boolean): void {
  ...
  setCacheStatsProvider(fn: () => Record<string, unknown>): void {
  ...
  toPrometheus(): string {
    ...
    if (this._cacheStatsProvider) {
      const cs = this._cacheStatsProvider() as Record<string, unknown>;
      const adapter = (cs?.adapter || {}) as Record<string, unknown>;
      if (adapter) {
        if (typeof adapter.keys === 'number') {
          lines.push('# HELP tmdb_cache_keys Current number of cached keys');
          lines.push('# TYPE tmdb_cache_keys gauge');
          lines.push(`tmdb_cache_keys ${adapter.keys}`);
        }
        if (typeof adapter.maxKeys === 'number') {
          lines.push('# HELP tmdb_cache_max_keys Maximum cache capacity');
          lines.push('# TYPE tmdb_cache_max_keys gauge');
          lines.push(`tmdb_cache_max_keys ${adapter.maxKeys}`);
        }
        if (typeof adapter.evictions === 'number') {
          lines.push('# HELP tmdb_cache_evictions_total Total cache evictions');
          lines.push('# TYPE tmdb_cache_evictions_total counter');
          lines.push(`tmdb_cache_evictions_total ${adapter.evictions}`);
        }
      }
      ...
      if (
        adapter &&
        typeof adapter.keys === 'number' &&
        typeof adapter.maxKeys === 'number' &&
        adapter.maxKeys > 0
      ) {
        const pct = (adapter.keys / adapter.maxKeys) * 100;
