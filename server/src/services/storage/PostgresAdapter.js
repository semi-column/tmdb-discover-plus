...
export class PostgresAdapter extends StorageInterface {
  ...
  async getUserConfig(userId) {
    const res = await this.pool.query('SELECT user_id, api_key_id, data FROM user_configs WHERE user_id = $1', [userId]);
  ...
  async getPublicStats() {
    ...
    const userResult = await this.pool.query('SELECT COUNT(DISTINCT api_key_id) as count FROM user_configs');
